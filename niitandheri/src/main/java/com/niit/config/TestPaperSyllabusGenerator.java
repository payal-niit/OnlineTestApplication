...
public class TestPaperSyllabusGenerator implements IdentifierGenerator {   
	...
	public Serializable generate(SessionImplementor session, Object object) throws HibernateException {
        ...
        try {
            // Oracle-specific code to query a sequence
            ps = connection.prepareStatement("SELECT payal.TestPaperSyllabusSequence.nextval AS testPaperSyllabusId FROM dual");
            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                int pk = rs.getInt("testPaperSyllabusId");
